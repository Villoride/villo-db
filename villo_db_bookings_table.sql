...
CREATE TABLE `bookings_table` (
  ...
  `status` enum('REQUESTED','SEARCHING','CONFIRMED','ARRIVING','IN_PROGRESS','COMPLETED','CANCELLED','NO_DRIVER') DEFAULT 'REQUESTED',
  ...
  PRIMARY KEY (`id`),
  KEY `driver_id` (`driver_id`,`user_id`),
  KEY `vehicle_id` (`vehicle_id`,`user_id`)
